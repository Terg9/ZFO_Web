...
function App() {
  ...
  useEffect(() => {
    async function fetchAllData() {
      try {
        ...
        const USDrate =
          currencyResponse.data.Valute.USD.Value.toFixed(4).replace('.', ',');
        const EURrate =
          currencyResponse.data.Valute.EUR.Value.toFixed(4).replace('.', ',');
    ...
    if (storedTasks) {
      try {
        ...
        if (Array.isArray(parsedTasks)) {
          setTodos(parsedTasks); // Восстановление задач
        } else {
  ...
  useEffect(() => {
    try {
      ...
      console.error('Ошибка при сохранении задач в localStorage:',
        error.message);
    }
